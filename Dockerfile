...
# Install ionCube
    ...
    && curl -O -L https://downloads.ioncube.com/loader_downloads/ioncube_loaders_lin_armv7l.zip \
    && PHP_EXT_DIR=$(php-config --extension-dir) \
    && unzip -j ioncube_loaders_lin_armv7l.zip ioncube/ioncube_loader_lin_7.4.so -d $PHP_EXT_DIR \
    && echo "zend_extension=ioncube_loader_lin_7.4.so" >> /usr/local/etc/php/conf.d/00_ioncube_loader_lin_7.4.ini \
    && rm -rf ioncube_loaders_lin_armv7l.zip \
# Install STL-THUMB
    && echo [Install STL-THUMB] \
    && curl -O -L https://github.com/unlimitedbacon/stl-thumb/releases/download/v0.3.1/stl-thumb_0.3.1_armhf.deb \
    && dpkg -i stl-thumb_0.3.1_armhf.deb \
    && rm -rf stl-thumb_0.3.1_armhf.deb \
